...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/ach1992/ACh-X-UI/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/ach1992/ACh-X-UI/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
